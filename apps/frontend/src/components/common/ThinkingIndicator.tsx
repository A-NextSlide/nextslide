/**
 * ThinkingIndicator
 * A fun, animated thinking indicator with a dancing star and rotating words
 * Used across chat interfaces to show AI processing state
 */
...
interface ThinkingIndicatorProps {
  /** Custom text to show instead of rotating words */
  customText?: string;
  /** Size variant */
  size?: 'sm' | 'md';
  /** Additional class names */
  className?: string;
  /** Whether to show the dancing star */
  showStar?: boolean;
  /** Interval for rotating words in ms (default: 10000) */
...
const DancingStar: React.FC<{ size: 'sm' | 'md' }> = ({ size }) => {
  const dimensions = size === 'sm' ? 14 : 18;

  return (
    <div className="dancing-star-wrapper">
      <svg
        width={dimensions}
        height={dimensions}
        viewBox="0 0 536 707"
        className="dancing-star"
      ...
      <style>{`
        ...
        .dancing-star {
          animation: starDance 2s ease-in-out infinite;
          filter: drop-shadow(0 0 3px rgba(255, 67, 1, 0.4));
        }
...
const ThinkingIndicator: React.FC<ThinkingIndicatorProps> = ({
  ...
  useEffect(() => {
    if (customText) return; // Don't rotate if custom text is provided
    ...
    return () => clearInterval(interval);
  }, [customText, rotationInterval]);

  const displayText = customText || THINKING_WORDS[wordIndex];
