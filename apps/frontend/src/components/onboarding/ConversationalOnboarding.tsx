    ...
    try {
      ...
      for await (const event of generator) {
        if (event.type === 'text') {
          ...
          if (outlineData?.scraped_videos) {
            console.log('[ConversationalOnboarding] 🎬 Received', outlineData.scraped_videos.length, 'scraped videos:', outlineData.scraped_videos);
          }
          setStatusMessage(null);
          setStatusPhase(null);
  ...
  const proceedWithGeneration = (slideMode: 'interactive' | 'static') => {
    ...
    setTimeout(() => {
      ...
      onComplete({
        ...
        // Pass scraped videos from website for use in deck generation
        scrapedVideos: outlineFlow?.scraped_videos,
      });
    }, 1500);
  };
  ...
  return (
    <div className="flex flex-col h-full max-w-2xl mx-auto w-full px-4 sm:px-6 lg:px-0">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-8 space-y-6">
        ...
        {/* Fallback: Show "Skip chat" button after 1+ user messages if no outline generated yet */}
        {!isAgentTyping && !isProcessing && (stage === 'conversing' || stage === 'chat') && messages.filter(m => m.role === 'user').length >= 1 && (
          <div className="flex flex-col items-center mt-4 animate-in fade-in">
            <button
              onClick={() => {
            ...
            <span className="text-xs text-zinc-400 mt-1">(i might miss some important details, answer below.)</span>
          </div>
        )}
