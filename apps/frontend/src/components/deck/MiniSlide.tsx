...
const MiniSlide: React.FC<MiniSlideProps> = ({
  ...
  // Extract background from slide components
  ...
  const hasContent = useMemo(() => {
    return Array.isArray(safeSlide.components) && safeSlide.components.length > 0;
  }, [safeSlide.components]);

  // MOBILE: Simple background-only rendering to prevent crashes
  if (isMobile) {
    return (
      <div
        ref={containerRef}
