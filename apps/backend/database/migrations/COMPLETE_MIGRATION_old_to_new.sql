-- COMPLETE MIGRATION: Remove Old Cache System, Add New Permanent Storage
-- Purpose: Clean transition from temporary cache to permanent brand storage
-- Run this in Supabase SQL Editor to completely replace the old system

-- ========================================
-- STEP 1: BACKUP ANY EXISTING CACHE DATA (Optional)
-- ========================================

-- Create a backup table of any existing cache data (optional, remove if not needed)
-- CREATE TABLE IF NOT EXISTS public.brandfetch_cache_backup AS 
-- SELECT * FROM public.brandfetch_cache WHERE success = true;

-- ========================================
-- STEP 2: DROP OLD CACHE SYSTEM COMPLETELY
-- ========================================

-- Drop the old cache statistics view
DROP VIEW IF EXISTS public.brandfetch_cache_stats;

-- Drop old cache triggers first (they depend on functions)
DROP TRIGGER IF EXISTS trigger_brandfetch_cache_updated_at ON public.brandfetch_cache;

-- Now drop old cache functions
...
-- Drop old cache indexes (will be automatically dropped with table, but being explicit)
...
-- Drop the old cache table completely
DROP TABLE IF EXISTS public.brandfetch_cache CASCADE;

-- ========================================
-- STEP 3: CREATE NEW PERMANENT STORAGE SYSTEM
-- ========================================
...
-- ========================================
-- STEP 4: MIGRATE DATA FROM OLD CACHE (if backup was created)
-- ========================================

-- Uncomment this section if you created a backup and want to migrate data
/*
INSERT INTO public.brand_data (
    domain,
    brand_name,
...
SELECT 
    COALESCE(normalized_identifier, identifier) as domain,
    COALESCE(brand_name, '') as brand_name,
    COALESCE(logo_urls, '{}'::jsonb) as logos,
    COALESCE(colors, '{}'::jsonb) as colors,
    COALESCE(fonts, '{}'::jsonb) as fonts,
    COALESCE(company_info, '{}'::jsonb) as company_info,
    COALESCE(confidence_score, 0) as confidence_score,
    api_response as brandfetch_response,
    COALESCE(hit_count, 0) as access_count,
    created_at,
    last_accessed_at
FROM public.brandfetch_cache_backup
WHERE success = true
AND normalized_identifier IS NOT NULL
ON CONFLICT (domain) DO NOTHING;  -- Don't overwrite if already exists

-- Drop the backup table after migration
DROP TABLE IF EXISTS public.brandfetch_cache_backup;
*/

-- ========================================
-- STEP 5: ADD DOCUMENTATION AND PERMISSIONS
-- ========================================
...
WHERE table_name = 'brand_data' 
AND table_schema = 'public'
ORDER BY ordinal_position;

-- Show statistics
SELECT 
    'brand_data' as table_name,
...
AND routine_name IN ('increment_brand_access', 'upsert_brand_data', 'update_brand_data_updated_at')
UNION ALL
SELECT 
    'Views created' as table_name,
    COUNT(*) as total_records
FROM information_schema.views 
WHERE table_schema = 'public' 
AND table_name = 'brand_data_stats';

-- Final success message
SELECT 
    '✅ Migration Complete!' as message,
    'Old cache system removed' as old_system_status,
    'New permanent storage ready' as new_system_status,
    'Update your code to use PermanentBrandService' as next_step;