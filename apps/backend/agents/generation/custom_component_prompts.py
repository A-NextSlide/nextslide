...
def build_system_prompt(
    ...
    return (
        ...
        "- ⚠️ PANEL IMAGE SWITCHING (when applicable): When tabs/buttons switch content panels:\n"
        "  * Image use is OPTIONAL. Use images only if they improve comprehension.\n"
        "  * If images are used, the main display image should change with the active panel\n"
        ...
        "  * On tab click: update the content, and also update the image if the UI includes one\n"
        "  * Example (with images): mainImage.src = items[index].image; mainImage.alt = items[index].imageAlt;\n"
        "  * WRONG: One static image for all tabs when each tab is meant to show different visual context.\n"
        "  * 🚨 If your UI uses switchable images, you MUST write `img.src = data.image;` — NEVER comment it out.\n"
        ...
        "  * 🚨 INIT ON LOAD: Always call selectTab(0) (or your equivalent) at the END of your <script>.\n"
        "    This ensures the image matches the initially active tab. Without it, the wrong image shows on load.\n\n"
        ...
        "2. JS ARRAYS (tabs, cards, sliders) — if images are used, each item gets its OWN image + imageAlt:\n"
        ...
        "□ If tabs/panels use images, they switch correctly on click\n"
        "□ Every item in a JS array has a UNIQUE imageAlt — no two items share the same description\n"
        "□ If image switching exists, tab/button handlers set img.src = data.image (NEVER commented out)\n"
        ...
        "□ Images are used only where they add value; text/icon/chart-only containers are allowed\n\n"
