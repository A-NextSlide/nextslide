...
def build_system_prompt(
    ...
    return (
        ...
        "HOVER/SCALE RULES (critical for smooth interactions):\n"
        "- Parent containers MUST have overflow-visible when children use hover:scale\n"
        "- Keep scale values small (1.02-1.05) to prevent overlap/flickering\n"
        "- Use transition-transform, NOT transition-all, for hover effects\n"
        "- Add padding/gap between hoverable items so scaled elements don't overlap\n\n"
        ...
        "- Chart.js needs <canvas>, D3 needs <svg>\n\n"
        "BUTTON RULES:\n"
        "- Every <button> MUST have onclick or addEventListener - buttons without handlers do nothing\n"
        "- Wire up click handlers that actually change state/UI\n\n"
        "LAYOUT RULES:\n"
        "- Set html, body { margin:0; padding:0; width:100%; height:100%; overflow:hidden; }\n"
        "- Use a root container that fills the canvas; no implicit padding\n"
        "- Pick 1-2 interactions max; must support the story\n"
        "- ALL content and animations must stay within 1920x1080 bounds\n"
        "- Accordion/expand animations: use reasonable max-height (not 9999px)\n"
