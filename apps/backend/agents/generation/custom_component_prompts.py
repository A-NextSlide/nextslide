...
def build_system_prompt(
    ...
    # Shared image guidance - only use images when they add information
    image_guidance = (
        "IMAGES: Use <img src=\"placeholder\" alt=\"VISUAL SEARCH QUERY\">.\n"
        "  - **ONLY INCLUDE IMAGES WHEN THEY ADD INFORMATION**. No decorative filler.\n"
        "  - NO IMAGE IS BETTER THAN DECORATIVE FILLER. Strong typography + whitespace beats abstract visuals.\n"
        "  - AVOID: Abstract textures, neon lines, geometric patterns, light trails, gradients-as-content.\n"
        "  - If the slide is about data/metrics, show the DATA (charts, numbers) not decorative imagery.\n"
        "  - When in doubt, SKIP THE IMAGE. Use bold typography and layout instead.\n"
    ...
    if slide_mode == "static":
        return (
            ...
            "- ONLY include images that ADD INFORMATION. No decorative filler.\n"
            "- NO IMAGE > decorative abstract. Use typography and whitespace instead.\n"
            "- AVOID: abstract textures, neon lines, geometric patterns, light trails\n"
    ...
    return (
        ...
        "- NEVER let flex/grid children auto-grow beyond available space - set explicit max-heights\n\n"
        "CRITICAL CSS RULES (MUST FOLLOW):\n"
        "- ⚠️ ABSOLUTELY NEVER use `user-select: none` on *, body, or html. This is a HARD RULE with zero exceptions. It breaks text selection AND can interfere with click handling on interactive elements.\n"
        "- NEVER use `clip-path` on containers that have interactive children below them - clip-path pointer event clipping is unreliable across browsers\n"
        ...
        "   - ONLY include images that ADD INFORMATION. No decorative filler.\n"
        "   - NO IMAGE is better than abstract decoration. Use typography + whitespace.\n"
        "   - AVOID: abstract textures, neon lines, geometric patterns, light trails\n"
        ...
        "   - **object-fit: Choose based on image PURPOSE**:\n"
        "     * Use `object-fit:cover` for: background images, hero images, ambient/atmosphere photos, textures, scenic/landscape fills\n"
        "     * Use `object-fit:contain` for: logos, product photos, diagrams, charts, infographics, screenshots, maps, book covers, movie posters, album art, portraits of specific people, any image where the FULL content must be visible\n"
        "     * Rule of thumb: If cropping the image would lose important information, use CONTAIN. If the image is decorative/atmospheric, use COVER.\n"
        "   - Example (cover): <div style=\"width:300px;height:200px;overflow:hidden;position:relative;\"><img src=\"placeholder\" alt=\"aerial coral reef turquoise\" style=\"width:100%;height:100%;object-fit:cover;position:absolute;\"></div>\n"
        "   - Example (contain): <div style=\"width:250px;height:250px;overflow:hidden;position:relative;display:flex;align-items:center;justify-content:center;\"><img src=\"placeholder\" alt=\"company logo minimal design\" style=\"width:100%;height:100%;object-fit:contain;position:absolute;\"></div>\n"
        ...
        "LAYERING & POINTER EVENTS (CRITICAL - buttons must be clickable):\n"
        "- ⚠️ SIMPLE RULE: Interactive elements (buttons, tabs, sliders) MUST be at the TOP of the z-index stack. Always.\n"
        "  * Set z-index: 9999 on ANY container that holds buttons, tabs, or clickable elements\n"
        "  * Example: .tabs-nav { position: relative; z-index: 9999; }\n"
        "  * Example: .controls { position: relative; z-index: 9999; }\n"
        "- ⚠️ EVERY non-interactive element with position:absolute MUST have `pointer-events: none`\n"
        "  * This includes: fog layers, gradients, overlays, watermarks, patterns, corner decorations,\n"
        "    stamps, badges, paperclips, ribbons, tape effects, texture overlays, image gradient fades\n"
        "  * If it's not a button/tab/link/slider, add `pointer-events: none` to it\n"
        "  * Elements with negative top/left that extend into other areas are especially dangerous\n"
        "- Background: z-index 1-10; media: 20-30; content panels: 40-50; titles: 100+; INTERACTIVE: 9999\n\n"
