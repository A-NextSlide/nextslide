...
class SlidePromptBuilder:
    ...
    def _add_rich_tiptap_formatting(self, sections: List[str], context: SlideGenerationContext) -> None:
        ...
        sections.extend([
            ...
            "- **SECTION TITLES/HEADINGS**: ALWAYS use style.bold=true for section titles and list item headers",
            "- Emphasize KEYWORDS and NUMBERS by splitting into segments and applying style.bold=true and accent textColor",
            "- For bullet point lists: make the first few words (the topic/heading) bold, then continue with normal weight",
            ...
            "- **MINIMUM HEIGHT**: Text blocks MUST be at least 60px tall (absolute minimum for readability)",
            "- For single-line text: height should be 2-3x the fontSize (e.g., 24px font → 60-72px height)",
            "- For multi-line text: height >= fontSize × lines × 1.5 plus 20% buffer for padding",
            "- Small text blocks (height < 140px): enforce SINGLE-LINE text (no manual line breaks).",
            "- If text would wrap in a small box, do ONE of:",
            "  a) Increase the block height to the required multi-line height (minimum 100px for 2 lines), OR",
    ...
    def _add_chart_requirements(self, sections: List[str], context: SlideGenerationContext):
        ...
        sections.extend([
            ...
            f'    "title": "{context.slide_outline.title}",',
            '    "showLegend": false,',
            '    "axisBottom": {"tickRotation": 0, "legend": "X Axis Label"},  // ALWAYS 0 - NEVER -45!',
            '    "axisLeft": {"tickRotation": 0, "legend": "Y Axis Label"},  // ALWAYS 0 - NEVER -45!',
            ...
            "- 🚨 ROTATION: tickRotation MUST be 0 for both axes - NEVER -45 or rotated!",
    ...
    def _add_rag_context(self, sections: List[str], rag_context: Dict[str, Any], context: SlideGenerationContext):
        ...
        if "Chart" in predicted and not getattr(context, 'has_tabular_data', False):
            ...
            "🚨 LABELS: Keep axis labels readable; tickRotation MUST be 0 (horizontal) - NEVER -45 or rotated!",
            "🚨 AXIS CONFIG: axisBottom.tickRotation: 0 and axisLeft.tickRotation: 0 - ABSOLUTELY NO ROTATION!",
        ...
        if "CustomComponent" in predicted:
            ...
            sections.extend([
                ...
                "   - Clamp text: fontSize = Math.min(desired, Math.max(14, Math.floor(props.width / (text.length * 0.6))))",
                "",
        ...
        if "CustomComponent" in predicted:
            ...
            sections.extend([
                ...
                "- Use props.width and props.height directly for layout calculations",
                ...
                "",
                "Layout patterns:",
                "  * TIMELINES (horizontal): Use absolute positioning, spread items across 0% to 100% of width",
                ...
                "  * PROGRESS BARS: Use 80-90% of props.width, not just 200px",
                "",
            ...
            sections.extend([
                ...
                "  const maxValueHeight = Math.floor(height * (emphasis === 'hero' ? 0.7 : 0.5));",
                "  const valueSize = hasValue ? Math.min( Math.floor(width / Math.max(3, valueText.length * 0.6)), maxValueHeight ) : 0;",
                "  const labelSize = Math.min(36, Math.floor(width / 12));",
                "  const titleSize = title ? Math.min(48, Math.floor(width / Math.max(8, title.length * 0.5))) : 0;",
                ...
                "      var xPos = padding + (pt.x * (props.width - padding * 2)); // Calculate pixel position from percentage",
                "      return React.createElement('div', {",
                "        key: i,",
                ...
                "}",
                "```",
                "KEY: Use pt.x as a percentage (0.0 to 1.0) and multiply by (props.width - padding * 2) to get pixel position!"
            ])
