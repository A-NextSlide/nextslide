...
class HTMLInspiredSlideGenerator(ISlideGenerator):
    ...
    def _load_component_schemas(self) -> str:
        ...
        # Use the OPTIMIZED schemas that enforce CustomComponent usage
        ...
        self._component_schemas_cache = schemas
        logger.info(f"📦 Component schemas loaded from OPTIMIZED source ({len(schemas)} chars)")
        return schemas
    ...
    def _build_html_inspired_user_prompt_dynamic(
        ...
        # Extract style context (chat history/preferences) - MUST BE DONE EARLY
        style_context = ""
        try:
            # Try to get from deck_outline.notes or stylePreferences
            if hasattr(context.deck_outline, 'notes') and isinstance(context.deck_outline.notes, dict):
                style_context = context.deck_outline.notes.get('style_context', '')
            
            # If empty, try to get from RAG context if available (passed via context usually)
            if not style_context and hasattr(context, 'rag_context'):
                style_context = context.rag_context.get('style_context', '')
                
            # Fallback: check if it's in the raw theme dict
            if not style_context:
                style_context = theme_dict.get('style_context', '')
                
        except Exception as e:
            logger.warning(f"⚠️ Failed to extract style_context: {e}")
...
StatCard (for data):
{{"type": "StatCard", "props": {{"label": "Growth", "value": "+20%", "trend": "up"}}}}

BigTitle (for headers):
{{"type": "BigTitle", "props": {{"text": "Main Headline", "highlight": "Headline"}}}}

SmartImage (for visuals):
{{"type": "SmartImage", "props": {{"src": "placeholder", "caption": "Description"}}}}

Line/Lines (dividers only):
...
Output valid JSON component array now (using the 3 colors above EXACTLY + alignment props):"""
        ...
        # We format the dynamic part with the extracted style_context
        # Note: style_context is already injected via f-string above, so we don't need .format() for it specifically
        # unless we escaped it. But since we moved the definition up, the f-string will capture it directly!
