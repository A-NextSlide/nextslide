...
class SlideGeneratorV2(ISlideGenerator):
    ...
    async def _post_process_slide(
        ...
        # Central non-overlap enforcement (single pass, type-aware gaps)
        try:
            # Ignore background-like elements in overlap: Background, Lines, and Shape act as containers/backdrops
            components_no_bg = [c for c in (slide_data.get('components', []) or []) if c.get('type') not in ('Background','Lines','Shape')]
            # Trace: pre non-overlap snapshot
            try:
                def _s_comp(c):
                    p = (c.get('props') or {})
                    pos = p.get('position') or {}
                    return f"{c.get('id')}:{c.get('type')}@({pos.get('x')},{pos.get('y')}) {p.get('width')}x{p.get('height')}"
                logger.info("[LAYOUT_TRACE] stage=pre_non_overlap count=%s components=%s",
                            len(components_no_bg),
                            ", ".join(_s_comp(c) for c in components_no_bg[:25]))
            except Exception:
                pass
            def _rect(c):
                p = (c.get('props') or {})
                ...
                def _s(v, d=0):
                    try:
                        ...
                        return d
                # Use top-left semantics for AABB math
                left = _s(pos.get('x', 0), 0)
                top = _s(pos.get('y', 0), 0)
                # For icons, fall back to size when width/height missing
                base_w = p.get('width', p.get('size', 0))
                base_h = p.get('height', p.get('size', 0))
                w = _s(base_w, 0)
                h = _s(base_h, 0)
            ...
            def _overlap(a, b):
                ax, ay, aw, ah = a
                bx, by, bw, bh = b
                return aw > 0 and ah > 0 and bw > 0 and bh > 0 and not (ax + aw <= bx or bx + bw <= ax or ay + ah <= by or by + bh <= ay)
            def _gap(t1, t2):
                heavy = {'Chart', 'Image', 'CustomComponent'}
                return 60 if (t1 in heavy or t2 in heavy) else 40
            order = list(range(len(components_no_bg)))
            order.sort(key=lambda i: (components_no_bg[i].get('props', {}).get('position', {}).get('y') or 0))
            GRID_Y = 24
            CANVAS_H = 1080
            EDGE = 80
            # Optional clamp helper when theme content area is present
            ca_top = ca_bottom = None
            try:
                posn = {}
                if isinstance(theme_structure, dict):
                    posn = theme_structure.get('positioning', {}) or {}
                ca = posn.get('content_area', {}) or {}
                if ca:
                    ca_top = int(ca.get('y', 220) or 220)
                    ch_local = int(ca.get('height', 600) or 600)
                    ca_bottom = ca_top + ch_local
            except Exception:
                ca_top = ca_bottom = None

            def _clamp_y(comp: Dict[str, Any], y_val: int, height_val: int) -> int:
                ctype = comp.get('type')
                props_local = (comp.get('props') or {})
                role = str((props_local.get('metadata') or {}).get('role') or '').lower()
                # Titles (by type or role) stay within a top header band
                if ctype in ('Title', 'Subtitle', 'Heading') or role in ('title', 'subtitle', 'heading'):
                    header_top = EDGE
                    header_bottom = 220
                    return max(header_top, min(y_val, header_bottom))
                # Allow sources/citations footers to sit near the bottom edge regardless of content area
                if role in ('sources', 'citations'):
                    return max(EDGE, min(y_val, max(EDGE, CANVAS_H - height_val - EDGE)))
                # Text blocks inside content area: constrain within content area fully (prevents sinking under content)
                if ca_top is not None and ca_bottom is not None and ctype in ('TiptapTextBlock', 'TextBlock'):
                    # Keep text within [ca_top, ca_bottom - height]
                    return max(ca_top, min(y_val, max(EDGE, ca_bottom - height_val)))
                # Default clamp within slide edges
                return max(EDGE, min(y_val, max(EDGE, CANVAS_H - height_val - EDGE)))

            # Treat separate left/right columns as independent to avoid unnecessary vertical pushes
            LCOL_RIGHT = 960  # Treat exact midline as separator
            RCOL_LEFT = 960
            def _separate_columns(a, b) -> bool:
                ax, ay, aw, ah = a
                bx, by, bw, bh = b
                a_right = ax + aw
                b_right = bx + bw
                return (a_right <= LCOL_RIGHT and bx >= RCOL_LEFT) or (b_right <= LCOL_RIGHT and ax >= RCOL_LEFT)

            def _is_title(comp: Dict[str, Any]) -> bool:
                ctype = comp.get('type')
                role = str(((comp.get('props') or {}).get('metadata') or {}).get('role') or '').lower()
                if ctype in ('Title',):
                    return True
                if role == 'title':
                    return True
            ...
            def _is_text(comp: Dict[str, Any]) -> bool:
                return comp.get('type') in ('TiptapTextBlock', 'TextBlock')

            def _paired_icon_ids(comp: Dict[str, Any]) -> list[str]:
                try:
                    meta = ((comp.get('props') or {}).get('metadata') or {})
                    return [pid for pid in (meta.get('pairedIconIds') or []) if isinstance(pid, str)]
                except Exception:
                    return []

            # Fast id->component map for syncing icon rows
            id_to_comp = {c.get('id'): c for c in (slide_data.get('components', []) or [])}

            def _has_left_icon_pair(text_rect, text_comp) -> bool:
                tx, ty, tw, th = text_rect
                t_bottom = ty + th
                # Scan for an icon immediately to the left with vertical overlap
                for cc in components_no_bg:
                    if cc.get('type') != 'Icon':
                        continue
                    ip = (cc.get('props') or {})
                    ipos = ip.get('position') or {}
                    iw = int(ip.get('width') or ip.get('size') or 24)
                    ih = int(ip.get('height') or ip.get('size') or 24)
                    ix = int(ipos.get('x') or 0)
                    iy = int(ipos.get('y') or 0)
                    i_right = ix + iw
                    i_bottom = iy + ih
                    # Horizontal: icon sits within 8..80px to the left of the text
                    if not (i_right <= tx - 8 and (tx - i_right) <= 80):
                        continue
                    # Vertical overlap
                    if not (iy < t_bottom and ty < i_bottom):
                        continue
                    return True
                return False

            def _column_fit_text(c, rect) -> bool:
                """Shrink text width to fit within its column to avoid cross-column overlap.
                Returns True if a change was made (and rect is updated by caller)."""
                if not _is_text(c):
                    return False
                px, py, pw, ph = rect
                # If this text crosses the midline from the left, clamp width
                crosses_mid = (px < LCOL_RIGHT) and (px + pw > LCOL_RIGHT)
                if not crosses_mid:
                    return False
                min_width = 400
                target_right = LCOL_RIGHT - 24
                new_w = max(min_width, target_right - px)
                if new_w >= pw:
                    return False
                try:
                    (c.setdefault('props', {}))['width'] = int(new_w)
                    logger.info("[LAYOUT_TRACE] stage=non_overlap action=column_fit target=%s type=%s before_w=%s after_w=%s",
                                c.get('id'), c.get('type'), pw, new_w)
                except Exception:
                    pass
                rect[2] = int(new_w)
                return True
            ...
            for ai in range(len(order)):
                i = order[ai]
                ci = components_no_bg[i]
                ti = ci.get('type')
                ri = _rect(ci)
                for bi in range(ai + 1, len(order)):
                    j = order[bi]
                    cj = components_no_bg[j]
                    tj = cj.get('type')
                    rj = _rect(cj)
                    # If text crosses midline, try width-fit instead of vertical push
                    if _is_text(cj):
                        _column_fit_text(cj, rj)
                    if _is_text(ci):
                        _column_fit_text(ci, ri)

                    if _overlap(ri, rj) and not _separate_columns(ri, rj):
                        gap = _gap(ti, tj)
                        if rj[1] >= ri[1]:
                            # Trace before adjust (j)
                            try:
                                pj = (cj.get('props') or {})
                                pposj = pj.get('position') or {}
                                before_y = pposj.get('y')
                            except Exception:
                                before_y = None
                            # If later component is a title, keep it in header band and move earlier component instead
                            if _is_title(cj):
                                alt_new_y_i = rj[1] + rj[3] + gap
                                if alt_new_y_i % GRID_Y != 0:
                                    alt_new_y_i += (GRID_Y - (alt_new_y_i % GRID_Y))
                                alt_new_y_i = _clamp_y(ci, int(alt_new_y_i), ri[3])
                                ci.setdefault('props', {}).setdefault('position', {})['y'] = int(alt_new_y_i)
                                ri[1] = int(alt_new_y_i)
                                try:
                                    pi = (ci.get('props') or {})
                                    pposi = pi.get('position') or {}
                                    after_y_i = pposi.get('y')
                                    logger.info("[LAYOUT_TRACE] stage=non_overlap action=push_down target=%s type=%s before_y=%s after_y=%s gap=%s (priority=title)",
                                                ci.get('id'), ti, before_y, after_y_i, gap)
                                except Exception:
                                    pass
                                continue
                            # If later component is text paired with left icon, prefer width fit over vertical push
                            if _is_text(cj) and _has_left_icon_pair(rj, cj):
                                if _column_fit_text(cj, rj):
                                    # Recompute overlap after column fit
                                    if not _overlap(ri, rj) or _separate_columns(ri, rj):
                                        logger.info("[LAYOUT_TRACE] stage=non_overlap action=column_fit_resolved target=%s", cj.get('id'))
                                        continue
                            # If earlier component is heavy and later is a text+icon row, push the heavy instead of the text
                            if ti in {'Chart', 'Image', 'CustomComponent'} and _is_text(cj) and _has_left_icon_pair(rj, cj):
                                alt_new_y_i_heavy = rj[1] + rj[3] + gap
                                if alt_new_y_i_heavy % GRID_Y != 0:
                                    alt_new_y_i_heavy += (GRID_Y - (alt_new_y_i_heavy % GRID_Y))
                                alt_new_y_i_heavy = _clamp_y(ci, int(alt_new_y_i_heavy), ri[3])
                                ci.setdefault('props', {}).setdefault('position', {})['y'] = int(alt_new_y_i_heavy)
                                ri[1] = int(alt_new_y_i_heavy)
                                try:
                                    pi = (ci.get('props') or {})
                                    pposi = pi.get('position') or {}
                                    after_y_i_heavy = pposi.get('y')
                                    logger.info("[LAYOUT_TRACE] stage=non_overlap action=push_down target=%s type=%s before_y=%s after_y=%s gap=%s (prefer_heavy_move)",
                                                ci.get('id'), ti, before_y, after_y_i_heavy, gap)
                                except Exception:
                                    pass
                                continue
                            # Prefer moving heavy component (charts/images/custom) instead of paired text+icon rows
                            heavy_types = {'Chart', 'Image', 'CustomComponent'}
                            # If cj is heavy and ci is text with a left icon pair, push cj down instead of cj's text or icons
                            if tj in heavy_types and _is_text(ci) and _has_left_icon_pair(ri, ci):
                                alt_new_y_j = ri[1] + ri[3] + gap
                                if alt_new_y_j % GRID_Y != 0:
                                    alt_new_y_j += (GRID_Y - (alt_new_y_j % GRID_Y))
                                alt_new_y_j = _clamp_y(cj, int(alt_new_y_j), rj[3])
                                cj.setdefault('props', {}).setdefault('position', {})['y'] = int(alt_new_y_j)
                                rj[1] = int(alt_new_y_j)
                                try:
                                    pj = (cj.get('props') or {})
                                    pposj = pj.get('position') or {}
                                    after_y = pposj.get('y')
                                    logger.info("[LAYOUT_TRACE] stage=non_overlap action=push_down target=%s type=%s before_y=%s after_y=%s gap=%s (prefer_heavy_move)",
                                                cj.get('id'), tj, before_y, after_y, gap)
                                except Exception:
                                    pass
                                continue
                            new_y = ri[1] + ri[3] + gap
                            if new_y % GRID_Y != 0:
                                new_y += (GRID_Y - (new_y % GRID_Y))
                            new_y = _clamp_y(cj, int(new_y), rj[3])
                            # If clamping prevented movement (stuck at or above ri.y), try moving the earlier component up
                            if int(new_y) <= int(ri[1]):
                                alt_y_i = rj[1] - ri[3] - gap
                                if alt_y_i % GRID_Y != 0:
                                    alt_y_i += (GRID_Y - (alt_y_i % GRID_Y))
                                alt_y_i = _clamp_y(ci, int(alt_y_i), ri[3])
                                if alt_y_i < ri[1]:
                                    # Set top-left y directly
                                    ci.setdefault('props', {}).setdefault('position', {})['y'] = int(alt_y_i)
                                    ri[1] = int(alt_y_i)
                                else:
                                    # Last resort: reduce gap to fit j just below i within bounds
                                    reduced_gap = max(16, gap // 2)
                                    alt_new_y = ri[1] + ri[3] + reduced_gap
                                    if alt_new_y % GRID_Y != 0:
                                        alt_new_y += (GRID_Y - (alt_new_y % GRID_Y))
                                    alt_new_y = _clamp_y(cj, int(alt_new_y), rj[3])
                                    cj.setdefault('props', {}).setdefault('position', {})['y'] = int(alt_new_y)
                                    # If cj is a text row, sync its paired icons by the same delta
                                    try:
                                        if _is_text(cj):
                                            before = rj[1]
                                            delta = int(alt_new_y) - int(before)
                                            if delta:
                                                for pid in _paired_icon_ids(cj):
                                                    ic = id_to_comp.get(pid)
                                                    if isinstance(ic, dict):
                                                        ip = ic.setdefault('props', {})
                                                        ipos = ip.setdefault('position', {})
                                                        ipos['y'] = int((ipos.get('y') or 0) + delta)
                                    except Exception:
                                        pass
                                    rj[1] = int(alt_new_y)
                            else:
                                # Write top-left y directly
                                cj.setdefault('props', {}).setdefault('position', {})['y'] = int(new_y)
                                try:
                                    if _is_text(cj):
                                        before = rj[1]
                                        delta = int(new_y) - int(before)
                                        if delta:
                                            for pid in _paired_icon_ids(cj):
                                                ic = id_to_comp.get(pid)
                                                if isinstance(ic, dict):
                                    ...
                                    pass
                                rj[1] = int(new_y)
                            # Trace after adjust (j)
                            try:
                                pj = (cj.get('props') or {})
                                pposj = pj.get('position') or {}
                                after_y = pposj.get('y')
                                logger.info("[LAYOUT_TRACE] stage=non_overlap action=push_down target=%s type=%s before_y=%s after_y=%s gap=%s",
                                            cj.get('id'), tj, before_y, after_y, gap)
                            except Exception:
                                pass
                        else:
                            # Trace before adjust (i)
                            try:
                                pi = (ci.get('props') or {})
                                pposi = pi.get('position') or {}
                                before_y_i = pposi.get('y')
                            except Exception:
                                before_y_i = None
                            # If earlier component is a title, keep it fixed and move the later component instead
                            if _is_title(ci):
                                alt_new_y_j = ri[1] + ri[3] + gap
                                if alt_new_y_j % GRID_Y != 0:
                                    alt_new_y_j += (GRID_Y - (alt_new_y_j % GRID_Y))
                                ...
                                try:
                                    pj = (cj.get('props') or {})
                                    pposj = pj.get('position') or {}
                                    after_y_j = pposj.get('y')
                                    logger.info("[LAYOUT_TRACE] stage=non_overlap action=push_down target=%s type=%s before_y=%s after_y=%s gap=%s (priority=title)",
                                                cj.get('id'), tj, before_y_i, after_y_j, gap)
                                except Exception:
                                    pass
                                continue
                            # If earlier component is text with left icon pair, prefer width fit over pushing it down
                            if _is_text(ci) and _has_left_icon_pair(ri, ci):
                                if _column_fit_text(ci, ri):
                                    if not _overlap(ri, rj) or _separate_columns(ri, rj):
                                        logger.info("[LAYOUT_TRACE] stage=non_overlap action=column_fit_resolved target=%s", ci.get('id'))
                                        continue
                            # Prefer moving heavy component when ci is heavy and cj is a text+icon row
                            heavy_types = {'Chart', 'Image', 'CustomComponent'}
                            if ti in heavy_types and _is_text(cj) and _has_left_icon_pair(rj, cj):
                                alt_new_y_i = rj[1] + rj[3] + gap
                                if alt_new_y_i % GRID_Y != 0:
                                    alt_new_y_i += (GRID_Y - (alt_new_y_i % GRID_Y))
                                alt_new_y_i = _clamp_y(ci, int(alt_new_y_i), ri[3])
                                ...
                                try:
                                    ...
                                    after_y_i = pposi.get('y')
                                    logger.info("[LAYOUT_TRACE] stage=non_overlap action=push_down target=%s type=%s before_y=%s after_y=%s gap=%s (prefer_heavy_move)",
                                                ci.get('id'), ti, before_y_i, after_y_i, gap)
                                except Exception:
                                    pass
                                continue
                            new_y = rj[1] + rj[3] + gap
                            if new_y % GRID_Y != 0:
                                new_y += (GRID_Y - (new_y % GRID_Y))
                            new_y = _clamp_y(ci, int(new_y), ri[3])
                            # If clamping prevented movement (stuck at or above rj.y), try moving the later component down instead
                            if int(new_y) <= int(rj[1]):
                                alt_y_j = ri[1] - rj[3] - gap
                                if alt_y_j % GRID_Y != 0:
                                    alt_y_j += (GRID_Y - (alt_y_j % GRID_Y))
                                alt_y_j = _clamp_y(cj, int(alt_y_j), rj[3])
                                if alt_y_j < rj[1]:
                                    cj.setdefault('props', {}).setdefault('position', {})['y'] = int(alt_y_j)
                                    rj[1] = int(alt_y_j)
                                else:
                                    # Last resort: reduce gap and try placing i just below j within bounds
                                    reduced_gap = max(16, gap // 2)
                                    alt_new_y_i = rj[1] + rj[3] + reduced_gap
                                    if alt_new_y_i % GRID_Y != 0:
                                        alt_new_y_i += (GRID_Y - (alt_new_y_i % GRID_Y))
                                    alt_new_y_i = _clamp_y(ci, int(alt_new_y_i), ri[3])
                                    ci.setdefault('props', {}).setdefault('position', {})['y'] = int(alt_new_y_i)
                                    try:
                                        if _is_text(ci):
                                            before_i = ri[1]
                                            delta_i = int(alt_new_y_i) - int(before_i)
                                            if delta_i:
                                                for pid in _paired_icon_ids(ci):
                                                    ic = id_to_comp.get(pid)
                                                    if isinstance(ic, dict):
                                                        ip = ic.setdefault('props', {})
                                                        ipos = ip.setdefault('position', {})
                                                        ipos['y'] = int((ipos.get('y') or 0) + delta_i)
                                    except Exception:
                                        pass
                                    ri[1] = int(alt_new_y_i)
                            else:
                                ci.setdefault('props', {}).setdefault('position', {})['y'] = int(new_y)
                                try:
                                    if _is_text(ci):
                                        before_i = ri[1]
                                        delta_i = int(new_y) - int(before_i)
                                        if delta_i:
                                            for pid in _paired_icon_ids(ci):
                                                ic = id_to_comp.get(pid)
                                                if isinstance(ic, dict):
        ...
        # Trace: post non-overlap snapshot
        try:
            comps_all = (slide_data.get('components', []) or [])
            def _s2(c):
                p = (c.get('props') or {})
                pos = p.get('position') or {}
                return f"{c.get('id')}:{c.get('type')}@({pos.get('x')},{pos.get('y')}) {p.get('width')}x{p.get('height')}"
            logger.info("[LAYOUT_TRACE] stage=post_non_overlap count=%s components=%s",
                        len(comps_all), ", ".join(_s2(c) for c in comps_all[:25]))
        except Exception:
            pass
        ...
        for component in slide_data.get('components', []):
            if component.get('type') in ('TiptapTextBlock', 'TextBlock', 'Title'):
                ...
                if 'letterSpacing' not in props:
                    ...
                    texts = props.get('texts', []) or []
                    max_size = max((t.get('fontSize', 0) for t in texts), default=(props.get('fontSize') or 0))
                ...
                texts = props.get('texts', []) or []
                if texts:
                    # Determine the biggest text segment as emphasis
