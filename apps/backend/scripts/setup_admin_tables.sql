...
-- 9. Create materialized view for user metrics (refresh periodically)
...
SELECT 
    ...
    COUNT(DISTINCT CASE WHEN d.status::text = 'completed' THEN d.uuid END) as completed_decks,
    COUNT(DISTINCT CASE WHEN d.status::text = 'draft' THEN d.uuid END) as draft_decks,
    COUNT(DISTINCT CASE WHEN d.visibility::text = 'public' THEN d.uuid END) as public_decks,
...
-- 13. Set up a cron job to refresh metrics (requires pg_cron extension)
-- SELECT cron.schedule('refresh-user-metrics', '*/30 * * * *', 'SELECT refresh_user_metrics();');

-- 14. Insert sample admin user (update the ID to match your admin user)
