...
class OutlineGenerator:
    ...
    def _slide_to_dict(self, slide: SlideContent) -> dict:
        ...
        logger.debug(f"[SLIDE_TO_DICT] Converting slide '{slide.title}' - has {tm_count} taggedMedia items")
        logger.debug(f"[SLIDE_TO_DICT] Converting slide '{slide.title}' - has {tm_count} taggedMedia items")
...
OUTPUT FORMAT: Return ONLY the slide content as bullet points. Do NOT add headers like 'SPEAKABLE CONTENT:', 'SPEAKER NOTES:', or 'CITATIONS:' - just the bullets. Citations go inline as [1], [2].
...
OUTPUT FORMAT: Return ONLY clean bullet points (•). NO section headers. NO metadata like 'SPEAKABLE CONTENT:', 'SPEAKER NOTES:', or 'CITATIONS:'. Put citations inline as [1], [2], [3].

🎯 FLEXIBLE MINIMAL - ADAPT TO CONTENT:
- Business/investor content: 3-5 bullets, 8-12 words each
- Simple topics: 2-3 bullets, 5-7 words each
- Test: Can you SPEAK this while presenting? If no, cut it
- NO paragraphs, NO section headers, NO long explanations

GOOD (speakable, data-rich):
• Q3 revenue: **$2.5B**, up **42%** YoY (8 words)
• Launched in **5 markets**, reached **12M users** (7 words)
• Indie games: **40%** PC market share in 2020 (8 words)

BAD (paragraphs, explanations):
❌ "Unity and Unreal Engine made professional-grade software free or affordable, eliminating barriers..." (paragraph - DELETE)
❌ "## Market Overview" (section header - DELETE)
...
CITE with [1], [2], [3]."""
                ...
                if isinstance(result, dict):
                    ...
                    if citations:
                        logger.info(f"[CITATIONS] Slide {idx+1} received {len(citations)} citations")
...
Make the data realistic and relevant to the topic. Use 4-8 data points."""
    ...
    def _enforce_word_limits_presentation(self, content: str, slide_title: str) -> str:
        """Enforce flexible minimal limits: MAX 12 words/bullet, MAX 5 bullets, MAX 80 words total."""
        if not content:
            return content
        ...
        logger.info(f"[WORD LIMIT] Slide '{slide_title}': {bullet_count} bullets, {total_words} total words")
        return result
        ...
        try:
            ...
            if options.detail_level == 'detailed':
                ...
                system = (
                    ...
                    "\n"
                    "CONTENT ADAPTATION:\n"
                    "• Business/investor: 3-5 bullets, 8-12 words each (concise facts)\n"
            ...
            if options.detail_level == 'detailed':
                ...
                content_bullet_limits = (
                    "Business/investor: 3-5 bullets, 8-12 words each (~60 words/slide). Simple topics: 2-3 bullets, 5-7 words (~30 words). Adapt to context. **Bold** numbers. NO paragraphs."
                )
            ...
            if options.detail_level != 'detailed':
                brevity_enforcement = (
                    ...
                    "• Business/investor content: 3-5 bullets, 8-12 words each (~60 words max/slide)\n"
                    "• Simple topics: 2-3 bullets, 5-7 words each (~30 words max/slide)\n"
            ...
            user += (
                ...
                f"FINAL: Adapt to content. Business/investor can have 5 bullets × 12 words. Simple topics: 3 bullets × 7 words. NO paragraphs. NO headers. {f'MAX 80 words/slide.' if options.detail_level != 'detailed' else 'Pack data.'}\n\n"
                ...
                "JSON: {title:str, slides:[{title:str, type:'title'|'content'|'stat'|'quote'|'chart', content:str, chart?:{chartType:str,data:[{name:str,value:num}]}}]}\n"
            )
            ...
            for s in slides_in:
                ...
                if options.detail_level != 'detailed' and slide_type not in ['title', 'quote', 'stat', 'divider']:
                    s_content = self._enforce_word_limits_presentation(s_content, s_title)
                ...
                if options.detail_level != 'detailed':
                    try:
                        ...
                        if slide_type in ['content'] and '[IMAGE:' not in s_content and '[image:' not in s_content.lower():
                            ...
                            s_content += f"\n[IMAGE: {image_desc}]"
                            logger.debug(f"[PRESENTATION] Added image tag to slide '{s_title}'")
                    except Exception as e:
